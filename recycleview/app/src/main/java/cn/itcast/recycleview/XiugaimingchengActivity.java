...
public class XiugaimingchengActivity extends AppCompatActivity {
    public static String phone_user;//用于接收数据库查询的返回数据
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mingcheng = findViewById(R.id.xiugainicheng);
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                try{
                    phone_user = connect.reachone(1);
                    phone_name = connect.reachthird(phone_user);
        ...
        if("修改名称".equals(mingcheng.getText().toString())){
                ...
                public void onClick(View v) {
                    Thread thread = new Thread(new Runnable() {
                        ...
                        public void run() {
                            if(!"修改名称".equals(mingcheng.getText().toString())){
                                try{
                                    connect.xiugainingcheng(phone_user,mingcheng.getText().toString());
                                }catch (SQLException e){
                                    e.printStackTrace();
                                }
                            }
