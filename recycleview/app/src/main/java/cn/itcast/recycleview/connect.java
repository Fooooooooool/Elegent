...
public class connect {
    ...
    public static int 	reviseone(final int id, final String phon) throws SQLException {//修改数据
        ...
        Statement stmt = conn.createStatement();
        //使用Connection来创建一个Statement对象
        String sql = "update phone set phone='" + phon + "' where id='" + id + "'";//修改的sql语句
        return stmt.executeUpdate(sql);//返回的同时执行sql语句，返回受影响的条目数量，一般不作处理
    }
...
//    public static int jiudianxiugai(final String name, final String number) throws SQLException {//修改数据
//        Connection conn = null;
//        conn = getConnection("person");
//        //使用DriverManager获取数据库连接
//        Statement stmt = conn.createStatement();
//        //使用Connection来创建一个Statement对象
//        String sql = "update jiudian set number='" + number + "' where name='" + name + "'";//修改的sql语句
//        return stmt.executeUpdate(sql);//返回的同时执行sql语句，返回受影响的条目数量，一般不作处理
//    }
    public static String jiudianreachthree(final String id) throws SQLException {//读取某一行
        ...
        //使用Connection来创建一个Statement对象
        ResultSet rs = stmt.executeQuery(
                "select number from jiudian where name='" + id + "'");
        rs.first();
        a = rs.getString(1);
        rs.close();
        return a;
